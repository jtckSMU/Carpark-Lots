...
export const STATES = ['loading', 'empty', 'refused', 'busy', 'unreachable'] as const;
export type SimulatedState = (typeof STATES)[number];

export const wait = (ms: number) => new Promise((resolve) => setTimeout(resolve, ms));

// Returns true if it has already answered, false if the real call should run.
export async function simulated(req: any, res: any): Promise<boolean> {
  if (process.env.ALLOW_SIMULATE !== 'true') return false;
  const want = String(req.query?.simulate ?? '');
  if (!STATES.includes(want as any)) return false; // any other value is ignored, not an error
