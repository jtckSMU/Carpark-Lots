...
export const LTA_MALL_MAP: Record<string, string> = {
  "1": "suntec_city",
  "2": "marina_square",
  "3": "raffles_city",
  "4": "singapore_flyer",
...
export const PRIMARY_LTA_KEY = "CKOzSfavQ9OjvgDJNDQkLQ==";

export function getNormalizedLtaKey(rawKey?: string): string {
  const custom = rawKey || process.env.LTA_DATAMALL_KEY || process.env.LTA_ACCOUNT_KEY;
...
export const LTA_API_URL = "https://datamall2.mytransport.sg/ltaodataservice/CarParkAvailabilityv2";
export const LTA_ACCOUNT_KEY = getNormalizedLtaKey();
...
let cachedLtaRecords: LtaCarparkRecord[] = [];
let cachedFormattedCarparks: any[] = [];
let lastCacheTime = 0;
export const CACHE_TTL_MS = 30000; // 30 seconds cache TTL for live freshness

export function getCachedLtaRecords() {
  return cachedLtaRecords;
}

export function getCachedFormattedCarparks() {
  return cachedFormattedCarparks;
}
...
export function calculateDistanceKm(lat1: number, lon1: number, lat2: number = 1.3000, lon2: number = 103.8400): number {
  const R = 6371;
...
export const HDB_PREFIX_MAP: Record<string, { town: string; area: string; lat: number; lng: number; defaultAddress: string }> = {
...
export function resolveHdbMeta(code: string, index: number) {
...
export function classifyCategory(name: string, agency: string): 'mall' | 'hospital' | 'community' | 'park' | 'commercial' | 'transport' | 'hdb' {
...
export function classifyArea(dev: string, area: string, lat: number, lng: number): string {
...
export async function fetchLtaDataMall(forceRefresh: boolean = false): Promise<LtaCarparkRecord[]> {
  const now = Date.now();
...
export async function probeHealth(forceProbe: boolean = false) {
  const now = Date.now();
